...
function showNotification(message, type = "success", ttl = 3500) {
  const containerId = "quickslot-notif-container";
  ...
  wrapper.className = `alert alert-${type} shadow-sm`;
  wrapper.role = "alert";
...
function sendConfirmationEmail(bookingPayload) {
  
  return new Promise((resolve) => {
    const summary = `${bookingPayload.provider} • ${bookingPayload.date} ${bookingPayload.slot}`;
    console.info("Simulating email send for booking:", bookingPayload);

    setTimeout(() => {
      console.info("Dummy email sent for:", bookingPayload.id);
      showNotification(`Confirmation email sent — ${summary}`, "success", 4000);
      resolve({ ok: true, message: "Simulated email delivered" });
    }, 800);
  });
}
...
confirmBtn.addEventListener("click", () => {
  ...
  console.warn("Email send simulation failed:", err);
  showNotification("Failed to send confirmation email (simulated).", "warning", 4000);
